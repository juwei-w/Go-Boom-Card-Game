...
public class Gui extends Application {
    ...
    Stage window;
    Label welcome, playerTurn, trickNum;
    ...
    public void start(Stage primaryStage) {
        ...
        // --- Content --- //
        trickNum = new Label("Trick #" + Boom.trickNum);
        ...
        drawCard.setOnAction(e -> {
            ...
            if (Boom.cards.size() != 0) {
                ...
                if (Boom.cards.size() == 0 && Boom.currentPlayer.playableCard == false) { // ----- Empty deck + No playable card
                    MessageBox.display("Empty Deck", "Deck is empty, player has no playable card, skips Player" + (Boom.players.indexOf(Boom.currentPlayer) + 1) + " turn.");
                    
                    Card emptyCard = new Card(" ", "nothing");
                    Boom.playedCenter.put(Boom.currentPlayer, emptyCard);
                    ...
                    MessageBox.display("Empty Deck", "Deck is empty");
                }
        ...
        save.setOnAction(e -> {
            ...
            if (filename != "cancel") {
                ...
                MessageBox.display("File saved.", "File is saved.");
            }
        });
        ...
        load.setOnAction(e -> {
            ...
            if (file.exists() && filename != "cancel") {
                Boom.loadGame(filename);
                updateGameState();
            }
            else if (filename == "cancel") {}
            else {
                MessageBox.display("File not found.", "File not found");
        ...
        // Trick Num
        trickNum.setPadding(new Insets(10));
        trickNum.setAlignment(Pos.CENTER);
        grid.add(trickNum, 1, 0, 1, 1);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            grid.add(new Label("Player " + (i + 1) + " : "), 0, i+1, 1, 1);
            updateOnHand(i);
            grid.add(player_cards[i], 1, i+1, 1, 1);
        }
    ...
    public void updateTrickNum() {
        trickNum.setText("Trick #" + Boom.trickNum);
    }
